...
class RigolMHO984:
    ...
    def set_input_impedance(self,channel, impedance="50"):
        
        """
        Set the input impedance of the specified channel.
        ...
        if impedance in("50", "50ohm", "50Ω"):
             self.scope.write(f":CHANnel{channel}:IMPedance FIFTy")
        else:
            self.scope.write(f":CHANnel{channel}:IMPedance OMEG")
    ...
    def get_measurements(self, channel):
        """
        Get automatic measurements for a channel using the MHO900/MHO984
        :MEASure:ITEM? syntax.
        ...
        Returns:
            dict: Dictionary containing measurement values. Unsupported or
                  unavailable measurements are returned as None.
        """
        ...
        # MHO900/MHO984 measurement item names.
        meas_types = {
            "vpp": "VPP",
            "vmax": "VMAX",
            "vmin": "VMIN",
            ...
            "rise_time": "RTIMe",
            "fall_time": "FTIMe",
            "duty": "PDUTy",       # Positive duty cycle
            "negative_duty": "NDUTy",
        ...
        # Set the default measurement source.
        self.scope.write(f":MEASure:SOURce {source}")

        for name, item in meas_types.items():
            ...
            try:
                ...
                # RIGOL commonly uses very large values to indicate an
                # unavailable/invalid automatic measurement.
                ...
                measurements[name] = None
                print(
                    f"Measurement unavailable: {name} "
                    f"({command}) -> {error}"
                )

        return measurements

    def read_error_queue(self, max_errors=20):
        """
        Read and clear the oscilloscope SCPI error queue.

        Returns:
            list[str]: Error messages reported by the oscilloscope.
        """
        errors = []

        for _ in range(max_errors):
            try:
                message = self.scope.query(":SYSTem:ERRor:NEXT?").strip()
            except Exception as error:
                errors.append(f"Unable to query error queue: {error}")
                break

            errors.append(message)

            # A zero error number indicates that the queue is empty.
            try:
                error_number = int(message.split(",", 1)[0])
    ...
    def print_error_queue(self, heading="Oscilloscope SCPI error queue"):
        """Print and clear all currently queued SCPI errors."""
        messages = self.read_error_queue()
