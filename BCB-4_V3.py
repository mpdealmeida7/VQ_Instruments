...
class BCB4Controller:
    ...
    def __init__(self, port: str, device_address: int = 1, timeout: float = 1.0):
        ...
        # Give the adapter a short time to settle
        time.sleep(0.5)
    ...
    def send_cmd(self, cmd: str) -> str:
        ...
        try:
            ...
            # Not all backends implement this; safe to ignore
            pass
        ...
        # Read one line (up to newline) within 'timeout'
        line = self.ser.readline().decode("ascii", errors="ignore").strip()
        return line

    # --------------------------
    # Device commands (string protocol)
    # --------------------------
    def set_device_address(self, new_address: int) -> str:
        ...
        # Only update local state after device accepts it
    ...
    def read_bias_voltage(self) -> str:
        """
        READ[ADDR]V -> Read bias voltage (raw ADC or formatted string per device)
        """
    ...
    def read_Vpi(self) -> str:
        """
        READ[ADDR]VPI -> Read Vpi voltage (raw ADC or formatted string per device)
        """
    ...
    def _countdown_wait(seconds: int) -> None:
        # Visible countdown during stabilization
    ...
    def run(self) -> None:
        """
        Interactive menu:
          1) Select bias mode and wait to stabilize (no reading)
          2) Read Bias and Vpi voltages (with simple calculations)
        Always closes the serial device at the end.
        """
        try:
            # Attempt a reset (optional)
            try:
            ...
            # Calibration constant and formulas (from your original code)
            ...
            if choice == 1:
                ...
                t = self._prompt_int("Enter stabilization wait time in seconds:", default=120)
                ...
                # Option 2: Read Bias and Vpi voltages now
                try:
                    raw_bias_str = self.read_bias_voltage().strip()
                    # If the device returns decorated strings, extract the number here:
                    # e.g., raw_bias_str = re.sub(r"[^0-9.\-]+", "", raw_bias_str)
            ...
            # Always close device connection
            try:
                self.close()
