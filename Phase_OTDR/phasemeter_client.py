...
try:
    # Only imported when actually connecting
...
class PhasemeterClient:
    ...
    def load(self, *, f0_hz: Optional[float] = None, configure_frontend: bool = True) -> None:
        """
        Connect and (optionally) pre-configure front-ends and PLL bandwidth.
        If f0_hz is provided, both channel PLLs are centered to that frequency.
        """
        if Phasemeter is None:
            raise RuntimeError(
                'moku package is not available in this environment. Install and retry.'
            )
    ...
    # ---------------------- On-the-fly settings ----------------------
    def set_frontend(self, channel: int, *, impedance: Optional[str] = None, coupling: Optional[str] = None, input_range: Optional[str] = None) -> None:
        if self._inst is None:
            raise RuntimeError('Device not loaded. Call load() first.')
        kwargs = {}
        if impedance is not None:
            kwargs['impedance'] = impedance
        if coupling is not None:
            kwargs['coupling'] = coupling
        if input_range is not None:
            kwargs['range'] = input_range
        if kwargs:
            self._inst.set_frontend(channel=channel, **kwargs)
    ...
    def read_phase_difference_deg(self) -> Tuple[float, dict]:
        """
        Get instantaneous Δφ = φ2 − φ1 in degrees and return (dphi_deg, raw_frame)
        """
    ...
    def live_plot_delta_phi_vs_time(self, *, window_s: float = 60.0, print_every_s: float = 1.0) -> None:
        """Live plot Δφ vs time with periodic console prints. Stop with Ctrl+C."""
        if self._inst is None:
            raise RuntimeError('Device not loaded. Call load() first.')
        plt.ion()
...
def sweep_phase_vs_frequency(
        ...
        *,
        ...
        average: str = 'median',  # 'median' or 'mean'
        reacquire_each_step: bool = True
    ) -> List[Tuple[float, float]]:
        """
        Sweep frequencies; at each f, measure Δφ and return list of (f_hz, dphi_deg).
        Both channels are set to the same PLL center frequency.
        """
        if self._inst is None:
            raise RuntimeError('Device not loaded. Call load() first.')
    ...
    def print_sweep(self, sweep_data: List[Tuple[float, float]]) -> None:
        """Pretty-print (f, Δφ) pairs to console."""
    ...
    def plot_sweep(self, sweep_data: List[Tuple[float, float]], *, title: str = 'Phase Difference vs Frequency', marker: str = 'o-') -> None:
        """Plot Δφ as a function of frequency."""
        if not sweep_data:
